-- ============================================================================
-- SCRIPT SQL: RESTAURAR v_matriz_liquidacion_horizontal A SU ESTADO ORIGINAL
-- ============================================================================
-- Este script elimina la vista anterior y la vuelve a crear en su versión limpia 
-- original sin las columnas de canal.
-- ============================================================================
...
SELECT 
    ...
    coalesce(m.puertos_comprobante, '-') AS PUERTO_COMPROBANTE,
    coalesce(m.puertos_fecha, '-') AS PUERTO_FECHA,
    
    -- OTROS
    coalesce(to_char(m.otros_monto, 'FM9999990.00'), '-') AS OTRO_MONTO,
    coalesce(m.otros_factura, '-') AS OTRO_FACTURA,
    coalesce(m.otros_fecha, '-') AS OTRO_FECHA,
    coalesce(m.otros_enlace, '-') AS OTRO_ALBO,
    coalesce(m.otros_cod, '-') AS "OTRO COD"
FROM public.mtodo m;
