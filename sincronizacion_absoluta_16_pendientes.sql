-- ==============================================================================
-- SCRIPT DE SINCRONIZACIÓN DEFINITIVO CON 1914-26 INCLUIDO COMO COMPLETADO
-- ==============================================================================
...
WHERE (COALESCE(NULLIF(regexp_replace(monto_total_factura::text, '[^0-9.]', '', 'g'), '')::numeric, 0) = 0)
  ...
  AND UPPER(TRIM(n_referencia)) <> '1914-26';

-- 5. Marcar estado_tramite = 'COMPLETADO' a despachos con factura/planilla o excepciones (Minera, EXP, 1914-26)
UPDATE public."GASTOS_DESPACHOS_2026"
...
WHERE (COALESCE(NULLIF(regexp_replace(monto_total_factura::text, '[^0-9.]', '', 'g'), '')::numeric, 0) > 0 
   ...
   OR (UPPER(TRIM(n_referencia)) = '1914-26');
