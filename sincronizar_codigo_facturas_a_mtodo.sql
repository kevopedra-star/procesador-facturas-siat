-- ==============================================================================
-- SCRIPT SQL: INCLUSIÓN DE 'COD' DE FACTURAS EN LA SINCRONIZACIÓN A MTODO
-- ==============================================================================
-- Obtiene el código de factura desglosado desde config_nits (o consecutivo)
-- y lo asigna automáticamente a albo_cod, ip_cod, guias_cod y otros_cod en mtodo.
-- ==============================================================================
...
BEGIN
    ...
    LEFT JOIN public.config_nits c ON c.nit = f.nit
    WHERE f.importacion_id = target_imp_id
      AND UPPER(f.tipo) IN ('IP', 'INSP PREVIA', 'INSPECCIÓN PREVIA');
    ...
    LEFT JOIN public.config_nits c ON c.nit = f.nit
    WHERE f.importacion_id = target_imp_id
      AND UPPER(f.tipo) IN ('GUIA', 'GUIAS', 'GUÍA', 'GUÍAS');
    ...
    FROM public.facturas f
    WHERE f.importacion_id = target_imp_id
      AND UPPER(f.tipo) IN ('PUERTO', 'PUERTOS');
    ...
    LEFT JOIN public.config_nits c ON c.nit = f.nit
    WHERE f.importacion_id = target_imp_id
      AND UPPER(f.tipo) NOT IN ('ALBO', 'DAB', 'IP', 'INSP PREVIA', 'INSPECCIÓN PREVIA', 'GUIA', 'GUIAS', 'GUÍA', 'GUÍAS', 'PUERTO', 'PUERTOS');
...
-- 2. Asegurar que el trigger siga activo en public.facturas
...
-- 3. Recalcular retroactivamente todas las facturas para poblar los CÓDIGOS inmediatamente
DO $$
DECLARE
    r RECORD;
BEGIN
